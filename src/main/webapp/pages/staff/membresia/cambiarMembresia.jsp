...
<head>
    ...
    <title>Cambiar Membresía · Golden Gym</title>
...
<body class="layout vista--membresia-cambiar">
...
<main class="layout__content">
    <section class="view card form-card membresia-registrar">
        <header class="view__header card__header">
            ...
            <h1 class="view__title titillium-negra">Cambiar Membresía</h1>
            <p class="view__sub">Cambiando membresía de <%= cliente.getNombre() %> <%= cliente.getApellido() %>.</p>
        </header>
        <% if (membresia == null) { %>
        ...
        <% } else if (request.getMethod().equals("POST")) {
            ...
            eventoDAO.agregarEventoMembresia(new EventoMembresia(-1, logueado.getId(), membresia.getId(), 5, Timestamp.from(Instant.now()), "web"));
        ...
        <div class="card__body">Se cambió la membresía con el plan <%= plan.getNombre() %>.<br>Fecha de inicio: <%= formateador.format(fechaDeInicio) %><br>Fecha de fin: <%= formateador.format(fechaDeFin)%></div>
         <% } else {
